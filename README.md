...
| **8-Layer Metadata Pipeline** | Chronological media and document date extraction. | EXIF DateTimeOriginal ➔ EXIF Digitized ➔ Image DateTime ➔ GPS UTC ➔ **MP4/MOV QuickTime `mvhd` Atom** (scanned at both the head *and* tail of the file, so non-faststart exports are covered) ➔ **PDF `CreationDate` Metadata** ➔ Filename Regex ➔ macOS Birthtime. Reads EXIF from **HEIC/HEIF, PNG, WebP and AVIF** as well as JPEG/TIFF/RAW, so iPhone photos are filed by capture date rather than by file modification time. |
...
| **Apple Live Photo Pairing** | Pairs `.heic` photos and `.mov` video clips together. | Pre-indexes HEIC dates using fast memory tuple keys `(dir_name, name_only)`. The still's capture date is **authoritative for the pair** and is consulted before the clip's own timestamps, so a Live Photo whose `.mov` has a reset mtime still lands beside its photo. |
| **Intact Code Repository Protection** | Detects development repositories (Git, Node, Python, Rust). | Preserves code folders 100% intact under `Code/project_name/`, **including `.git` history, file permissions and extended attributes** — an executable `build.sh` arrives still executable. Build artefacts (`node_modules`, `dist`, `__pycache__`, …) are skipped, but are **counted and listed in the preview** so nothing disappears silently. |
...
* **Free Space Pre-Flight**: A fresh run that cannot fit on the destination is refused before a single file is copied. Two cases warn instead of refusing: a resumed run (most data may already be there), and a same-volume transfer on APFS, where copy-on-write cloning means a copy consumes almost no additional space.
* **Nothing Skipped Silently**: Build and cache folders (`node_modules`, `.venv`, `build`, …) are excluded from the file count to keep transfers fast, but they are counted and listed in the preview so you can see exactly what was left behind.
* **Fast Resume**: Re-running against an existing destination performs a lightweight consistency check rather than re-hashing the entire destination, so a multi-terabyte merge starts moving immediately.
